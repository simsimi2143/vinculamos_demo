        ...
        success: function(resListar) {
            ...
            if (datosInfra.length == 0) {
                ...
                datosInfra.forEach(registro => {
                    $('#codigoinfra').append(new Option(registro.tinf_nombre, registro.tinf_codigo));
                });
            }
        },
        error: function(error) {
...
function buscarTipoInfra() {
    ...
    $.ajax({
        type: 'GET',
        url: window.location.origin+'/admin/crear-iniciativa/buscar-tipoinfra',
        data: {
...
function eliminarInfraestructura(inic_codigo, enti_codigo, tinf_codigo) {
        ...
        success: function(resListar) {
            ...
            if (datosRrhh.length == 0) {
                ...
                datosRrhh.forEach(registro => {
                    $('#codigorrhh').append(new Option(registro.trrhh_nombre, registro.trrhh_codigo));
                });
            }
        },
        error: function(error) {
        ...
        success: function(resListar) {
            ...
            datosRrhh.forEach(registro => {
                fila =  '<tr>'+
                            ...
                            '<td>'+
                                '<button type="button" class="btn btn-icon btn-sm btn-danger" onclick="eliminarRrhh('+registro.inic_codigo+', '+registro.enti_codigo+', '+registro.trrhh_codigo+')"><i class="fas fa-trash"></i></button>'+
                            '</td>'+
                        '</tr>';
